          ...
          <section className={`${handles.handles.container_section_1}`}>
            ...
            <div className={`${handles.handles.container_blocks_tabs}`}>
              {blocks.map((item: blocksI, index) => (
                <div
                  ref={containerClick}
                  key={'container_imgs_' + index}
                  onClick={() => handleShowTabs(item)}
                >
                  <div
                    className={`${handles.handles.container_imgs} ${handles.handles.container_img_1}`}
                  >
                    <img
                      className={`${handles.handles.img}`}
                      src={
                        isMobile ? item.thumbnailMobile : item.thumbnailImage
                      }
                      alt="Imagen de referencia"
                    />
                    <div
                      className={`${handles.handles.container_texts} ${handles.handles.container_text_1}`}
                    >
          ...
          <section className={`${handles.handles.container_section_2}`}>
            <div className={`${handles.handles.container_section_2_box}`}>
              <div className={`${handles.handles.split_box}`}>
                <div className={`flex ${handles.handles.container_tabs_items}`}>
                  <div className={`${handles.handles.container_btn_tab_close}`}>
                    <button
                      onClick={() => handleHideTabs()}
                      type="button"
                      className={`${handles.handles.btn_tab_close}`}
                    >
                      <svg
                        width="14"
                        height="20"
                      ...
                      >
                        <path
                          d="M6.59574 13.5106L0 6.91484L6.59574 0.319092L7.76649 1.48984L2.34149 6.91484L7.76649 12.3398L6.59574 13.5106Z"
                          fill="#707070"
                        />
                      </svg>
                    </button>
                  ...
                  {blocks.map((block, index) => (
                    <div
                      className={`${handles.handles.container_btn_tab_items} ${
                        handles.handles.container_btn_tab_items + '_' + index
                      }`}
                      key={'container_imgs_tabs' + index}
                    >
                      <button
                        className={`${handles.handles.btn_tab_items} ${
                          itemTab.title === block.title &&
                          handles.handles.tab_active
                        }`}
                        type="button"
                        onClick={() => handleShowTabs(block)}
                      >
              ...
              <div className={`${handles.handles.container_tabs_img}`}>
                <img
                  className={`${handles.tabs_img}`}
                  src={isMobile ? itemTab.imageMobile : itemTab.image}
                  alt=""
                />
              </div>
            </div>
