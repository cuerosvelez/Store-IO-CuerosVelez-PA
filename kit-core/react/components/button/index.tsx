...
const Button: React.FC<ButtonProps> = ({
  ...
  const currentSettings = useMemo(() => {
    if (isMobile) {
      ...
      return applySettings(desktopSettings[listPositionDesktop] || {});
    }
  }, [isMobile, listPositionMobile, listPositionDesktop]);
  ...
  const wrapperClass = useMemo(() => {
    ...
    const directionClass =
      isMobile || currentSettings.directionRow ? 'flex-column' : 'flex-row';
      ...
      {buttons.map(
        ({ url, name, newTab, colorSingle = color }: ButtonType, idx) => {
          ...
          const buttonStyle = {
            ...
            background:
              colorBackground ?? hoverState[`hover-${idx}`]
                ? `${colorSingle}61`
                : `${colorSingle}00`,
            border:
